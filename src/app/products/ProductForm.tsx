...
export function ProductForm({ id, defaultValues: d, isNew, deactivateButton }: Props) {
  ...
  return (
    <form className="space-y-4">
      ...
      <Field label="Purchase cost (£) — per retail unit">
      ...
      {showCaseSize && (
        <Field label={UNIT_CASE_LABELS[unit] ?? 'Units per delivery case'}>
          <input name="case_size" type="number" min="1" step="1"
            defaultValue={d.case_size} className="input-field"
            placeholder="e.g. 12" />
        </Field>
      )}
      {!showCaseSize && (
        <input type="hidden" name="case_size" value="1" />
      )}

      <div className="border-t border-white/10 pt-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] mb-3">Pricing Engine</p>
        <div className="space-y-3">
          <Field label="Price multiplier (retail ÷ cost — auto-calculated)">
