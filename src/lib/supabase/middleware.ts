...
export async function updateSession(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/delivery-note') || pathname.startsWith('/api/golem') || pathname.startsWith('/api/telegram-webhook') || pathname.startsWith('/api/integrity-check') || pathname.startsWith('/api/recalc-pricing')) {
    return supabaseResponse
  }
  ...
  // Fetch role and enforce cashier restriction
  ...
  if (profile?.role === 'cashier' && pathname !== '/wholesale-lookup') {
    const url = request.nextUrl.clone()
    url.pathname = '/wholesale-lookup'
    return NextResponse.redirect(url)
  }
